...
try:
    ...
    sql_spending_file_path = os.path.join(cur_dir, "Building2.sql")
    ...
    custom_labels = ['0', '50', '100', '150', '200']
    print(f"min(num_users): {min(num_users)}, max(num_users): {max(num_users)}, len(num_users): {len(num_users)}")
    plt.xticks(num_orders, labels=custom_labels, rotation=45)
    plt.tight_layout()

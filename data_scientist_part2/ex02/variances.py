...
class Variances:
    ...
    def run(self):
        ...
        try:
            ...
            num_components, explained_variances = self.select_features(
                sorted_variances, total_variance
            )
            self.plot_selected_variances(explained_variances, num_components)
            print(
                f"Number of components to explain 90% variance: {num_components} ou of {len(sorted_variances)}"
            )
